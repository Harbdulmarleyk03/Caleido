...
class TestAuthE2EFlow:
    def test_user_auth_flow(self, api_client):
        ...
        verify_email_response = api_client.get(
            "/api/v1/auth/verify-email/", {"token": str(token)}
        )
        ...
        logout_response = api_client.post(
            "/api/v1/auth/logout/", {"refresh": str(new_refresh)}
        )
        ...
        profile_response = api_client.get(
            "/api/v1/users/me/", {"old_access": str(access)}
        )
