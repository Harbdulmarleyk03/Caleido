...
class HealthService: 
    ...
    def check_database():
        ...
        try:
            ...
            if result != (1,):
                raise ServiceUnavailableError("Health Check query did not return the expected result.")
    ...
    def check_celery():
        try:
            ping_result = app.control.ping(timeout=settings.HEALTH_CHECK_TIMEOUTS['CELERY'])
        except OSError as e:
            raise ServiceUnavailableError("IOError") from e
        except NotImplementedError as e:
            raise ServiceUnavailableError("NotImplementedError: Make sure CELERY_RESULT_BACKEND is set") from e
        else:
            if not ping_result:
