...
class TestHealthCheck:
    ...
    def test_all_dependencies_healthy(self, mock_ping, mock_cache, mock_connections, api_client):
        mock_ping.return_value = [{"worker@host": {"ok": "pong"}}]
        mock_cursor = mock_connections.__getitem__.return_value.temporary_connection.return_value.__enter__.return_value
    ...
    def test_db_is_down(self, mock_ping, mock_cache, mock_connections, api_client):
        mock_ping.return_value = [{"worker@host": {"ok": "pong"}}]       
        mock_connections.__getitem__.return_value.temporary_connection.side_effect = db.Error("Connection refused")
        mock_cache.get.return_value = 'test-probe-key'
        with patch('apps.health.services.uuid.uuid4', return_value='test-probe-key'):
            response = api_client.get('/api/v1/health/')
    ...
    @patch('apps.health.services.cache')
    @patch('apps.health.services.app.control.ping')
    def test_two_dependencies_are_down(self, mock_ping, mock_cache, mock_connections, api_client):
        mock_connections.__getitem__.return_value.temporary_connection.side_effect = db.Error("down")
