...
def test_returns_utc_window_when_date_matches_rule(saturday_rules):
    ...
    window_start, window_end = get_availability_window(saturday_rules, target_date, owner_timezone)

    tz = pytz.timezone(owner_timezone)

    expected_start = tz.localize(datetime.combine(target_date, time(9, 0))).astimezone(pytz.UTC)
    expected_end = tz.localize(datetime.combine(target_date, time(17, 0))).astimezone(pytz.UTC)
