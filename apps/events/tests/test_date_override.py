...
class TestDateOverrideView:
    ...
    def test_list_authenticated_user(self, auth_client, event_type):
        url = reverse(
            "date-override-list-create", kwargs={"event_type_id": event_type.pk}
        )
    ...
    def test_retrieve_override(self, auth_client, date_override, event_type):
        url = reverse(
            "date-override-detail",
            kwargs={"event_type_id": event_type.pk, "pk": date_override.pk},
        )
        response = auth_client.get(url)
        assert response.status_code == 200
    ...
    def test_non_owner_cannot_retrieve(
        self, api_client, other_user, date_override, event_type
    ):
