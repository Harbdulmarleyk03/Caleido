...
class TestAvailabilityRuleView:
    ...
    def test_list_authenticated_user(self, auth_client, event_type):
        url = reverse('availability-rule-list-create', kwargs={'event_type_id': event_type.id})
    ...
    def test_update_rule(self, auth_client, availability_rule):
        ...
        assert availability_rule.start_time == "11:00:00"
    ...
    def test_non_owner_cannot_retrieve(self, api_client, other_user, availability_rule):
    ...
    def test_non_owner_cannot_update(self, api_client, other_user, availability_rule):
        api_client.force_authenticate(user=other_user)
        url = reverse('availability-rule-detail', kwargs={'event_type_id': event_type.pk, 'pk': availability_rule.pk})
        response = api_client.patch(url, {'start_time': '9:00:00'})
