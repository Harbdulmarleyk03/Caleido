...
class TestAvailabilityRuleView:
    ...
    def test_list_authenticated_user(self, auth_client, event_type):
        url = reverse('availability-rule-list-create', kwargs={'event_type_id': event_type.pk})
    ...
    def test_retrieve_rule(self, auth_client, availability_rule, event_type):
        url = reverse('availability-rule-detail', kwargs={'event_type_id': event_type.pk, 'pk': availability_rule.pk})
        response = auth_client.get(url)
        assert response.status_code == 200
    ...
    def test_non_owner_cannot_retrieve(self, api_client, other_user, availability_rule, event_type):
        api_client.force_authenticate(user=other_user)
        url = reverse('availability-rule-detail', kwargs={'event_type_id': event_type.pk, 'pk': availability_rule.pk})
        response = api_client.get(url)
        assert response.status_code == 404

    def test_non_owner_cannot_delete(self, api_client, other_user, availability_rule, event_type):
        api_client.force_authenticate(user=other_user)
        url = reverse('availability-rule-detail', kwargs={'event_type_id': event_type.pk, 'pk': availability_rule.pk})
