    ...
    def validate(self, data):
        ...
        if event_type:
            overlapping = AvailabilityRule.objects.filter(
                ...
                start_time__lt=data[
                    "end_time"
                ],  # existing rule starts before new one ends
                end_time__gt=data[
                    "start_time"
                ],  # existing rule ends after new one starts
            )
            ...
            if overlapping.exists():
                raise serializers.ValidationError(
                    "This time window overlaps with an existing rule."
                )
        return data
    ...
    def validate(self, data):
        ...
        if not is_unavailable:
            ...
            if custom_start is None or custom_end is None:
                raise serializers.ValidationError(
                    "Start time and end time are required when is_unavailable is False"
                )
        ...
        if event_type:
            ...
            existing = DateOverride.objects.filter(
                event_type=event_type, specific_date=data["specific_date"]
            )
