...
def generate_reschedule_token(booking):
    signed_token = signing.dumps({'booking_id': str(booking.id)}, salt='reschedule-booking')
    return signed_token

def verify_reschedule_token(token, max_age=604800):
    try:
        data = signing.loads(token, salt='reschedule-booking', max_age=max_age)
        booking = Booking.objects.get(id=data['booking_id'])
        return booking 
    except signing.SignatureExpired:
        raise ValueError("Token Expired")
