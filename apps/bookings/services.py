...
class BookingService:
    ...
    def create_booking(
        ...
        with transaction.atomic():
            ...
            audit_user = (
                user if user and user.is_authenticated else booking.event_type.owner
            )
            ...
            transaction.on_commit(
                lambda: send_booking_confirmation_email.delay(str(booking.id))
            )
            transaction.on_commit(
                lambda: BookingService.schedule_booking_reminder(str(booking.id))
            )
            return booking, True
    ...
    def cancel_booking(booking, user):
        with transaction.atomic():
            ...
            transaction.on_commit(
                lambda: (
                    app.control.revoke(task_id_24h, terminate=False)
                    if task_id_24h
                    else None
                )
            )
            transaction.on_commit(
                lambda: (
                    app.control.revoke(task_id_1h, terminate=False)
                    if task_id_1h
                    else None
                )
            )
            transaction.on_commit(
                lambda: send_booking_cancellation_email.delay(str(booking.id))
            )
    ...
    def reschedule_booking(booking, user, start_time, end_time):
        with transaction.atomic():
            ...
            transaction.on_commit(
                lambda: send_booking_reschedule_email.delay(str(booking.id))
            )
