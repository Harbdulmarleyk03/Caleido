...
class TestBookingIntegration:
    ...
    def test_owner_cancel_already_cancelled_booking(
        self, auth_client, owner, event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
            end_time=end,
    ...
    def test_non_owner_cannot_cancel_booking(
        self, auth_client, other_user, other_event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
            end_time=end,
    ...
    def test_invitee_cancels_with_valid_token(
        self, api_client, other_user, other_event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
            end_time=end,
    ...
    def test_invitee_cancels_with_expired_token(
        self, api_client, other_user, other_event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
    ...
    def test_invitee_uses_token_for_wrong_booking(
        self, api_client, other_user, event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
            end_time=end,
    ...
    def test_owner_reschedules_a_free_slot(self, auth_client, owner, event_type):
        ...
        data = {"start_time": start + timedelta(hours=2)}
        ...
        audit = BookingAudit.objects.filter(
            booking=booking, action="rescheduled"
        ).first()
        assert audit is not None
        assert audit.previous_data["status"] == "confirmed"
    ...
    def test_owner_reschedules_a_cancelled_booking(
        self, auth_client, owner, event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
    ...
    def test_invitee_reschedules_with_valid_token(
        self, api_client, other_user, other_event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
    ...
    def test_same_slot_as_current_booking(
        self, api_client, other_user, other_event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
        ...
        url = reverse("booking-reschedule", args=[booking.id])
        response = api_client.patch(
            url, {"token": str(token), "start_time": start.isoformat()}, format="json"
        )
        booking.refresh_from_db()
        assert response.status_code == 200

    def test_only_bookings_for_the_authenticated_user_event_types(
        self, auth_client, owner, event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
    ...
    def test_filter_by_start_time_range(self, auth_client, owner, event_type):
        ...
        response = auth_client.get(
            url,
            {
                ...
                "start_time_before": (start + timedelta(hours=2)).isoformat(),
            },
        )
        assert response.status_code == 200
        assert len(response.data["results"]) == 1
    ...
    def test_another_user_bookings_never_appear_in_the_response(
        self, auth_client, owner, event_type, other_user, other_event_type
    ):
        booking, _ = BookingService.create_booking(
            start_time=start,
            end_time=end,
