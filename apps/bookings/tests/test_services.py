...
class TestScheduleBookingReminder:
    ...
    def test_schedules_both_reminders_for_booking_more_than_24_hours_away(self, apply_async, booking_with_invitee):
        ...
        apply_async.assert_any_call(args=[str(booking_with_invitee.id), "24h"], eta=reminder_24h_eta)
        apply_async.assert_any_call(args=[str(booking_with_invitee.id), "1h"], eta=reminder_1h_eta)
    ...
    @patch('apps.bookings.services.send_booking_reminder_email.apply_async')
    def test_does_not_schedule_reminders_for_imminent_booking(self, apply_async, booking_with_invitee):
